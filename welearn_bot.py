...
# Helper function to get server response as a dictionary
def get_response(session, url, **data):
...
def get_courses_by_id():
    ...
    courses = get_response(s, server_url, wstoken=token, moodlewsrestformat="json", wsfunction="core_course_get_courses_by_field")
...
with Session() as s:
    ...
    login_response = get_response(s, login_url, username=username, password=password, service="moodle_mobile_app")
    try:
    ...
    if action == "whoami":
        ...
        info = get_response(s, server_url, wstoken=token, moodlewsrestformat="json", wsfunction="core_webservice_get_site_info")
        print(info['fullname'])
        sys.exit(0)
        ...
        courses = get_response(s, server_url, wstoken=token, moodlewsrestformat="json", wsfunction="core_enrol_get_users_courses", userid=userid)
        ...
        assignments = get_response(s, server_url, wstoken=token, moodlewsrestformat="json", wsfunction="mod_assign_get_assignments")
        ...
        for course in assignments['courses']:
            ...
            for assignment in course['assignments']:
                ...
                submission = get_response(s, server_url, wstoken=token, moodlewsrestformat="json", wsfunction="mod_assign_get_submission_status", \
        ...
        urls = get_response(s, server_url, wstoken=token, moodlewsrestformat="json", wsfunction="mod_url_get_urls_by_courses")
        ...
        resources = get_response(s, server_url, wstoken=token, moodlewsrestformat="json", wsfunction="mod_resource_get_resources_by_courses")
